...
class SearchController
{
    ...
    public function index()
    {
        ...
        if (! app('cache')->has($key)) {
            ...
            // cache
            app('cache')->put($key, $data, $this->cache_time);
            app('redis')->set($key, $data);

        }
ddd('Search Index:',timer()-START);
    ...
    {
        $key = 'goods.search';
        $key .= is_null($this->cid) ? '' : ':cid:' . $this->cid;
        $key .= is_null($this->brand_id) ? '' : ':brand_id:' . $this->brand_id;
        $key .= is_null($this->keywords) ? '' : ':keywords:' . $this->keywords;
        $key .= ':sort:' . $this->sort;
        $key .= ':page:' . $this->page;
        $key .= ':per_page:' . $this->per_page;
    ...
    protected function filterGoodsIds($ids)
    {
        ...
        usort($ids, function($a, $b) use ($direction){
            $a = Goods::getGoodsCell($a, $this->sort);
            $b = Goods::getGoodsCell($b, $this->sort);
    ...
    protected function getGoodsByIds($ids)
    {
        ...
        foreach ($ids as $id){
            $goods[] = [
                'id' => Goods::getGoodsCell($id, 'id'),
                'name' => Goods::getGoodsCell($id, 'name'),             // 商品名
                'thumb' => Goods::getGoodsCell($id, 'thumb'),            // 缩略图
                'orig' => Goods::getGoodsCell($id, 'orig'),             // 原价
                'price' => Goods::getGoodsCell($id, 'price'),            // 销售价
                'quantity' => Goods::getGoodsCell($id, 'quantity'),         // 数量
                'sales' => Goods::getGoodsCell($id, 'sales'),            // 销量
                'sort' => Goods::getGoodsCell($id, 'sort'),             // 排序
                'only_sku' => Goods::getGoodsCell($id, 'only_sku'),         // 单一SKU
                'only_sku_id' => Goods::getGoodsCell($id, 'only_sku_id'),    // 单一SKU id
                'tag_thumb' => Goods::getGoodsCell($id, 'tag_thumb'),        // 缩略图标签
                'tag_prom' => Goods::getGoodsCell($id, 'tag_prom'),         // 优惠促销标签
                'features' => Goods::getGoodsCell($id, 'features')          // 特征卖点标签
            ];
        }
